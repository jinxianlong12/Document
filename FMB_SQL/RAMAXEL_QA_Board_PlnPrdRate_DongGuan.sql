...
from(
  select
       TO_CHAR(to_date(H.TRAN_TIME,'YYYY-MM-DD HH24MISS'),'YYYY-MM') WORK_DATE,
       SUM(H.QTY_1) PROD_QTY,
       SUM(CASE WHEN H.TRAN_CODE = 'COLLECT_DFT' THEN QTY_1 ELSE 0 END) DEFT_QTY,
       (SELECT SUM(M.ORD_QTY) FROM MWIPORDSTS M where  SUBSTR(M.PLAN_START_TIME,0,6) = TO_CHAR(sysdate,'yyyymm') and M.ORD_CMF_4 in('100050','100010','100020') )  AS PLAN_QTY
  from MWIPLOTHIS H
