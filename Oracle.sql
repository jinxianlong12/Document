--------------------------------------------字符串函数--------------------------------------------
SELECT concat('010-','88888888')||'23' phoneNo
FROM dual;
-------------010-8888888823, concat是添加两个字符串
SELECT initcap('smith hEllo') upp
FROM dual;
-------------Smith Hello
...
FROM dual;
-------------AABBCCDD
SELECT lpad(rpad('gao',10,'*'),17,'*')
FROM dual;
-------------*******gao*******
SELECT replace('he lohe you','he','i')
FROM dual;
-------------i loi you
SELECT ltrim(rtrim('   gao qian jing   ',' '),' ')
FROM dual;
-------------gao qian jing
SELECT trim('a' FROM 'abacda')
FROM dual;
-------------bacd
SELECT ltrim('abaaaabbbcda','ab')
FROM dual;
-------------cda
SELECT substr('130123456789',3,8) 截取字符串
FROM dual;
-------------01234567
SELECT substr('aaaa-bbbb-cc',instr('aaaa-bbbb-cc','-',-1)+1)
FROM dual;
-------------cc    截取最后一个'-'后面的字符串
...
SELECT LENGTH (' 130 ') 返回字符串长度
FROM DUAL;
-------------返回5, 空格也算
SELECT FLOOR (2345.67), FLOOR (-2345.67)
FROM dual;
-------------2345, -2346, 返回小于等于x的最大整数值
SELECT mod(10,3), mod(3,3), mod(2,0)
FROM dual;
-------------1  0  2, 返回x除以y的余数.如果y是0,则返回x
SELECT power(2,10), power(3,3)
FROM dual;
-------------1024   27
SELECT ROUND (55.655, 2), ROUND (55.654, 2), ROUND (45.654, -1), ROUND (45.654, -2), ROUND (55.654, -2)
FROM DUAL;
-------------55.66               55.65             50                 0                  100
SELECT sign(123), sign(-100), sign(0)
FROM dual;
-------------1,-1,0
SELECT trunc(123.567,2), trunc(123.567,-2), trunc(123.567)
FROM dual;
-------------123.56          100                  123
SELECT sum(列明)
FROM 表名
group by 分组的列
-------------计算该列的总和, 按分组来显示
SELECT name, DECODE(admin_flag,0,'普通用户',1,'管理员','默认值')
FROM USERS;
-------------jinxianlon	管理员
-------------MY TEST	普通用户
...
--------------------------------------------日期函数--------------------------------------------
SELECT trunc (SYSDATE, 'DD'), trunc (SYSDATE, 'MM'), trunc (SYSDATE, 'yyyy'), trunc (SYSDATE, 'day'), trunc (SYSDATE, 'q')
FROM DUAL;
...
Select *
From GG_DTXX t
Where to_char(create_time,'yyyymm') = to_char(Sysdate,'yyyymm')
select *
from GG_DTXX t
...
select *
from GG_DTXX t
where t.create_time >=trunc(sysdate,'YYYY') and t.create_time<=add_months(trunc(sysdate,'YYYY'),12)-1
-------------当年：
...
SELECT to_char(add_months(to_date('199912','yyyymm'),2),'yyyymm')
FROM dual;
-------------200002
SELECT SYSDATE
FROM daul
...
SELECT months_between('19-12月-1999','19-3月-1999') mon_between
FROM dual;
-------------9, 两个日期之间的相差月数
SELECT next_day('18-8-2018','星期五') next_day
FROM dual;
-------------2018-08-24 00:00:00, 
...
--------------------------------------------转换--------------------------------------------
SELECT to_char(SYSDATE,'yyyy.mm.dd') aa
FROM dual;
-------------2018.08.18(转换成char类型)
SELECT to_date('19991201','yyyy-mm-dd')
FROM dual;
-------------1999-12-01 00:00:00(字符串转换成date类型)
SELECT to_number('1999') year
FROM dual;
-------------1999(转换成int类型)
...
FROM tbale_name;
-------------列值是null的话返回0
SELECT nvl(column_name,0,1)
FROM tbale_name;
-------------列值是null的话返回1, 不是null返回0v
SELECT
  TO_CHAR (TRUNC(ADD_MONTHS(LAST_DAY (SYSDATE), -1) + 1)+ ROWNUM- 1,'YYYYMMDD') AS WORK_DATE
FROM DUAL
CONNECT
BY ROWNUM<TRUNC
(TRUNC
...
TRUNCATE qrtz_blob_triggers, qrtz_triggers, qrtz_job_details,qrtz_cron_triggers,qrtz_simple_triggers,qrtz_simprop_triggers RESTART IDENTITY
