SELECT * FROM(
SELECT
month, WORK_DATE ,SHIFT,AREA,WORK_SHOP,USER_ID,USER_DESC,
TOTAL_TIME ,  --投入工时
PROD_TIME,    --产出工时
EXC_TIME ,    --异常/间接工时
ACT_TIME ,    --实做工时
...
(
    ...
    (
        ...
        where WORK_DATE >= NVL(TRIM('20180601'),WORK_DATE)
         AND WORK_DATE <= NVL(TRIM('20180628'),WORK_DATE)
         AND delete_flag != 'Y'
         AND AREA IN ('DG-SDT','DG-SSD','HY-MEM')
         AND WORK_SHOP = NVL(TRIM('SMT'),WORK_SHOP)
         AND AREA = NVL(TRIM('DG-SSD'),AREA)
    ...
    (
        ...
        where WORK_DATE >= NVL(TRIM('20180601'),WORK_DATE)
         AND WORK_DATE <= NVL(TRIM('20180628'),WORK_DATE)
         AND ACT_TIME_TYPE = '1'  AND delete_flag != 'Y'
         AND AREA IN ('DG-SDT','DG-SSD','HY-MEM')
         AND WORK_SHOP = NVL(TRIM('SMT'),WORK_SHOP)
         AND AREA = NVL(TRIM('DG-SSD'),AREA)
         AND USER_ID = NVL(TRIM(' '),USER_ID)
    ...
    (
        ...
        where WORK_DATE >= NVL(TRIM('20180601'),WORK_DATE)
         AND WORK_DATE <= NVL(TRIM('20180628'),WORK_DATE)
         AND ACT_TIME_TYPE = '2' AND delete_flag != 'Y'
         AND AREA IN ('DG-SDT','DG-SSD','HY-MEM')
         AND WORK_SHOP = NVL(TRIM('SMT'),WORK_SHOP)
         AND AREA = NVL(TRIM('DG-SSD'),AREA)
         AND USER_ID = NVL(TRIM(' '),USER_ID)
    ...
    (
        ...
        where WORK_DATE >= NVL(TRIM('20180601'),WORK_DATE)
         AND WORK_DATE <= NVL(TRIM('20180628'),WORK_DATE)
         AND ACT_TIME_TYPE = '3' AND delete_flag != 'Y'
         AND AREA IN ('DG-SDT','DG-SSD','HY-MEM')
         AND WORK_SHOP = NVL(TRIM('SMT'),WORK_SHOP)
         AND AREA = NVL(TRIM('DG-SSD'),AREA)
         AND USER_ID = NVL(TRIM(' '),USER_ID)
...
WHERE 1=1
AND DIR_EFF BETWEEN 110 AND 120
